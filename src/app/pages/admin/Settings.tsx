...
import { Lock, Trash2, AlertTriangle, Eye, EyeOff, Mail, CheckCircle, Shield } from 'lucide-react';
...
export default function AdminSettings() {
  ...
  const handleForgotPassword = () => {
    if (!forgotEmail.trim()) { toast.error('Please enter email/username'); return; }
  ...
  return (
    <DashboardLayout role="admin">
      <div className="space-y-8 max-w-2xl mx-auto">
        <div>
          <h2 className="text-3xl text-slate-900">Admin Settings</h2>
          <p className="text-slate-600 mt-1">Manage admin account security</p>
        </div>
        ...
        <Card className="bg-gradient-to-br from-blue-50 to-white border-blue-200">
          <CardContent className="pt-4 pb-4">
            <div className="flex items-center gap-4">
              <div className="w-16 h-16 rounded-full bg-gradient-to-br from-blue-600 to-blue-800 flex items-center justify-center text-white text-2xl">
                A
              </div>
              <div>
                ...
                <div className="flex items-center gap-1 mt-1">
                  ...
                  <span className="text-xs text-blue-600">Platform Administrator</span>
                </div>
              </div>
            </div>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <Button className="bg-blue-600 hover:bg-blue-700 w-full" onClick={handleChangePassword}>
              Update Password
            </Button>
          </CardContent>
        </Card>
          ...
          <CardContent>
            {!showForgotFlow ? (
              <div className="flex items-center justify-between">
                <p className="text-sm text-slate-600">Send password reset instructions to your admin email.</p>
              ...
              <div className="space-y-3">
                <Input placeholder="Admin email or username" value={forgotEmail} onChange={(e) => setForgotEmail(e.target.value)} />
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {[
              ...
              { label: 'SMS alerts for support ticket submissions', defaultOn: true },
              { label: 'Daily donation summary report', defaultOn: true },
              { label: 'Auto-approve verified NGOs', defaultOn: false },
            ].map((pref, i) => (
              <div key={i} className="flex items-center justify-between py-2 border-b border-slate-100 last:border-0">
                <span className="text-sm text-slate-700">{pref.label}</span>
                <button
                  className={`w-12 h-6 rounded-full transition-colors ${pref.defaultOn ? 'bg-blue-500' : 'bg-slate-300'}`}
            ...
            {!showDeleteConfirm ? (
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <p className="text-slate-700 mb-1">Permanently delete your admin account</p>
                  <p className="text-sm text-slate-500 mb-4">Warning: Deleting the admin account will remove all admin controls. This action cannot be undone.</p>
              ...
              <AnimatePresence>
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                  <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                    <p className="text-red-800 text-sm font-semibold">⚠️ Critical: This will delete the admin account</p>
                    <p className="text-red-600 text-sm mt-1">Enter your password to confirm:</p>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <Button className="bg-red-600 hover:bg-red-700 flex-1" onClick={handleDeleteAccount}>Confirm Delete</Button>
